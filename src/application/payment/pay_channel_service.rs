...
pub async fn list(search: PayChannelSearch) -> Result<Vec<PayChannelRes>> {
    let db = DB().await;
    ...
    let list = pay_channel::Entity::find()
        .filter(condition)
...
pub async fn add(arg: PayChannelAdd) -> Result<String> {
    let db = DB().await;
...
pub async fn edit(arg: PayChannelEdit) -> Result<String> {
    let db = DB().await;
    let channel = pay_channel::Entity::find_by_id(arg.id)
        .one(db)
        .await?
...
pub async fn delete(id: i64) -> Result<String> {
    let db = DB().await;
    let channel = pay_channel::Entity::find_by_id(id)
        .one(db)
        .await?
        .ok_or_else(|| Error::not_found("渠道不存在"))?;
...
pub async fn update_config(arg: PayChannelConfig) -> Result<String> {
    let db = DB().await;
    let channel = pay_channel::Entity::find_by_id(arg.id)
        .one(db)
        .await?
...
pub async fn toggle_status(id: i64) -> Result<String> {
    let db = DB().await;
    let channel = pay_channel::Entity::find_by_id(id)
        .one(db)
        .await?
