    ...
    <a-form layout="vertical">
      ...
      <template v-if="configData.linkageEnabled">
        <a-form-item label="联动配置">
          <a-textarea v-model="configData.linkageConfig" placeholder="请输入联动配置（JSON格式）" :auto-size="{ minRows: 3 }" />
        </a-form-item>
      </template>
      ...
      <a-form-item label="自定义样式">
        <a-textarea v-model="configData.customStyle" placeholder="请输入自定义样式（CSS）" :auto-size="{ minRows: 2 }" />
      </a-form-item>
    </a-form>
  </div>
</template>
