    ...
    >
      <template v-if="schema.groups && schema.groups.length > 0">
        <div v-for="group in visibleGroups" :key="group.id" class="form-group">
          ...
          <a-row :gutter="16">
            <a-col v-for="field in getVisibleFields(group.fields)" :key="field.id" :span="getFieldSpan(field)">
              <FormField
                :field="field"
      ...
      <template v-else>
        <a-row :gutter="16">
          <a-col v-for="field in visibleFields" :key="field.id" :span="getFieldSpan(field)">
            <FormField
              :field="field"
              :model-value="formData[field.name]"
              :disabled="isFieldDisabled(field)"
