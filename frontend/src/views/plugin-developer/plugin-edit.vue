<template>
  <div class="snow-page">
          ...
          <a-card title="基本信息">
            <a-form ref="formRef" :model="form" layout="vertical">
              ...
              <a-form-item field="categoryId" label="插件分类" :rules="[{ required: true, message: '请选择插件分类' }]">
                <a-cascader v-model="form.categoryId" :options="categoryOptions" placeholder="请选择插件分类" allow-search check-strictly />
              </a-form-item>
          ...
          <a-card title="验证级别" style="margin-top: 16px">
            <a-form ref="verifyFormRef" :model="form" layout="vertical">
              ...
              <a-form-item label="验证说明">
                <a-alert v-if="form.verifyLevel === 0" type="info">
                  无需验证，插件可被任意用户安装使用
                </a-alert>
                ...
                <a-alert v-else type="warning">
                  高级验证：启用设备绑定、到期时间校验、API调用次数限制等完整功能
                </a-alert>
              </a-form-item>
            </a-form>
          </a-card>
    ...
    <a-modal v-model:visible="uploadModalVisible" title="上传图片" :width="500" @ok="onConfirmUpload" @cancel="uploadModalVisible = false">
      <a-form layout="vertical">
...
const categoryOptions = ref([
  { value: 1, label: '营销工具', children: [
    { value: 11, label: '优惠券' },
    { value: 12, label: '秒杀' },
    { value: 13, label: '拼团' },
  ]},
  { value: 2, label: '客服系统', children: [
    { value: 21, label: '在线客服' },
    { value: 22, label: '工单系统' },
  ]},
  { value: 3, label: '数据分析', children: [
    { value: 31, label: '数据统计' },
    { value: 32, label: '用户分析' },
  ]},
]);
