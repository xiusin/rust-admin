...
const tableData = ref<FormConfigItem[]>([]);
const models = ref<CmsModel[]>([]);
const previewVisible = ref(false);
const previewConfig = ref<FormConfigDetail | null>(null);
...
const getList = async () => {
  ...
  loading.value = true;
  try {
    tableData.value = await formApi.list(searchForm.modelId);
  } catch (error) {
    console.error(error);
  } finally {
    loading.value = false;
...
const onPreview = async (record: FormConfigItem) => {
  try {
    previewConfig.value = await formApi.detail(record.id);
