...
import { isNil, clamp, minInArray, isFunction, isNumber, isString, Logger, clamper, toValidNumber, pickWithout } from "@visactor/vutils";
...
export class SankeyLayout {
    ...
    layout(data, config) {
        ...
        isVertical(this.options.direction)) if (this.options.inverse) {
    ...
    computeHierarchicNodeLinks(originalNodes) {
        ...
        const doSubTree = (subTree, depth, parents) => {
            subTree.forEach(((node, index) => {
                const nodeKey = this._getNodeKey ? this._getNodeKey(node) : parents ? `${parents[parents.length - 1].key}-${index}` : `${depth}-${index}`, nodeValue = isNil(node.value) ? 0 : toValidNumber(node.value);
