...
const hierarchy_1 = require("./hierarchy"), vutils_1 = require("@visactor/vutils"), vgrammar_util_1 = require("@visactor/vgrammar-util");
...
class SankeyLayout {
    ...
    layout(data, config) {
        ...
        (0, vgrammar_util_1.isVertical)(this.options.direction) ? this._viewBox = {
            x0: viewBox.y0,
            x1: viewBox.y1,
            y0: viewBox.x0,
            y1: viewBox.x1,
        ...
        (0, vgrammar_util_1.isVertical)(this.options.direction)) if (this.options.inverse) {
    ...
    computeHierarchicNodeLinks(originalNodes) {
        ...
        (0, hierarchy_1.calculateNodeValue)(originalNodes);
        const doSubTree = (subTree, depth, parents) => {
            subTree.forEach(((node, index) => {
                const nodeKey = this._getNodeKey ? this._getNodeKey(node) : parents ? `${parents[parents.length - 1].key}-${index}` : `${depth}-${index}`, nodeValue = (0, 
