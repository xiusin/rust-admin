...
class Event {
    ...
    on(eType, query, callback) {
        ...
        if (events_1.ComposedEventMapper[eType]) {
            const composedEvent = new events_1.ComposedEventMapper[eType](this._eventDispatcher, this._mode);
    ...
    off(eType, query, cb) {
        ...
        if (events_1.ComposedEventMapper[eType]) if (callback) null === (_a = this._composedEventMap.get(callback)) || void 0 === _a || _a.event.unregister(), 
