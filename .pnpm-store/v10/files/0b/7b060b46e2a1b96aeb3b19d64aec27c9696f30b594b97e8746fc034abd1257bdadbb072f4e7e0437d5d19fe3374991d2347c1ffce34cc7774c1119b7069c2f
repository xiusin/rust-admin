...
class BaseChart extends compilable_base_1.CompilableBase {
    ...
    setLayoutTag(tag, morphConfig, renderNextTick = !0) {
        ...
        return (null == compiler ? void 0 : compiler.getVGrammarView()) && (compiler.getVGrammarView().updateLayoutTag(), 
        tag && renderNextTick && compiler.renderNextTick(morphConfig)), this._layoutTag;
    }
    getGlobalScale() {
    ...
    layout(params) {
        ...
        this.getLayoutTag()) {
            ...
            this._layoutFunc(this, elements, this._layoutRect, this._viewBox), this._event.emit(event_2.ChartEvent.afterLayout, {
                elements: elements,
                chart: this
            }), this.setLayoutTag(!1), this.onLayoutEnd(params), this._event.emit(event_2.ChartEvent.layoutEnd, {
                chart: this,
                vchart: this._option.globalInstance
            });
        }
    ...
    updateComponentSpec(result) {
        const componentCache = {}, checkVisibleComponents = {
            ...
            [interface_1.ComponentTypeEnum.mapLabel]: !0,
    ...
    clearState(state) {
        this.getAllRegions().forEach((r => {
            r.interaction.clearEventElement(state, !0), r.interaction.resetInteraction(state, null);
        }));
    }
    clearAllStates() {
        this.getAllRegions().forEach((r => {
        ...
        [ event_2.ChartEvent.dataZoomChange, event_2.ChartEvent.scrollBarChange ].forEach((event => {
            this._event.on(event, (({value: value}) => {
                ...
                const enableMarkAnimate = () => {
                    this._enableMarkAnimation([ "exit", "update" ]), this._event.off(event_2.VGRAMMAR_HOOK_EVENT.AFTER_MARK_RENDER_END, enableMarkAnimate);
                };
                this._event.on(event_2.VGRAMMAR_HOOK_EVENT.AFTER_MARK_RENDER_END, enableMarkAnimate);
            }));
        }));
    }
    ...
    filterGraphicsByDatum(datum, opt = {}) {
        var _a;
        const keys = (datum = datum ? (0, util_1.array)(datum) : null) ? Object.keys(datum[0]) : null, allElements = [], getDatumOfElement = null !== (_a = opt.getDatum) && void 0 !== _a ? _a : el => el.getDatum();
        return this.getRegionsInQuerier(opt.region).forEach((r => {
            const pickElements = [];
            datum && r.getSeries().forEach((s => {
                s.getMarks().forEach((m => {
                    if (m.getProduct() && (!opt.filter || (0, vutils_1.isFunction)(opt.filter) && opt.filter(s, m))) {
