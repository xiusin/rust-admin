...
const vutils_1 = require("@visactor/vutils"), vrender_core_1 = require("@visactor/vrender-core"), attribute_1 = require("./attribute");

vrender_core_1.Animate.mode |= vrender_core_1.AnimateMode.SET_ATTR_IMMEDIATELY;

let GlobalAnimatorId = 0;

const isCustomAnimateCtor = custom => {
    var _a;
    if ((0, vutils_1.isNil)(custom) || (0, vutils_1.isNil)(custom.prototype)) return !1;
    const prototype = null !== (_a = custom.prototype) && void 0 !== _a ? _a : {};
    return prototype instanceof vrender_core_1.ACustomAnimate || "onBind" in prototype && "onStart" in prototype && "onEnd" in prototype && "onUpdate" in prototype;
};
...
class Animator {
    ...
    animateTimeSlice(graphicAnimate, timeSlice, animationParameters, parameters) {
        ...
        if (delay > 0 && graphicAnimate.wait(delay), effects.length < 0) graphicAnimate.wait(duration); else {
            ...
            1 === customAnimates.length ? graphicAnimate.play(customAnimates[0]) : customAnimates.length > 1 && graphicAnimate.play(new vrender_core_1.AnimateGroup(duration, customAnimates));
        }
