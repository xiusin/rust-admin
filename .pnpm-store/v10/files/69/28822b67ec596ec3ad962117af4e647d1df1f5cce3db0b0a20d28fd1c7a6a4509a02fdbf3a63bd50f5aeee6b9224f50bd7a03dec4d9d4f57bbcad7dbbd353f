...
export class FunnelSeries extends BaseSeries {
    ...
    _computeOuterLabelLinePosition(datum) {
        ...
        let x1, x2, y1, y2;
        if (this._isHorizontal()) {
            const spaceWidth = null !== (_m = outerLabelSpec.spaceWidth) && void 0 !== _m ? _m : FUNNEL_LABEL_SPACE_WIDTH, points = this.getPoints(datum), shapeMiddleHeight = (Math.abs(points[0].y - points[1].y) + Math.abs(points[2].y - points[3].y)) / 2;
