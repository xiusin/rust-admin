...
const vutils_1 = require("@visactor/vutils"), base_1 = require("./base"), vgrammar_util_1 = require("@visactor/vgrammar-util");
...
class GridLayout extends base_1.BaseLayout {
    ...
    initGrid(config) {
        ...
        if ((0, vutils_1.isObject)(shape)) {
            const canvas = (0, vgrammar_util_1.generateMaskCanvas)(shape, config.width, config.height);
            ...
            let i, isEmptyPixel = (0, vgrammar_util_1.generateIsEmptyPixel)(shape.backgroundColor);
    ...
    layout(data, config) {
        ...
        const {center: center, maxRadius: maxRadius} = (0, vgrammar_util_1.getMaxRadiusAndCenter)(this.options.shape, [ config.width, config.height ]);
