...
export class Mark extends GrammarBase {
    ...
    update(spec) {
        var _b;
        if (this.emit(HOOK_EVENT.BEFORE_MARK_UPDATE), this._context = this.spec.context, 
        ...
        this.markType !== GrammarMarkType.group) {
            ...
            isNil(spec.overflow) || this.graphicItem.setAttribute("overflow", spec.overflow), 
    ...
    evaluateJoin(data) {
        ...
        isCollectionMark ? this.differ.setCurrentData(this._groupedData) : this.differ.setCurrentData(groupData(currentData, (datum => `${groupKeyGetter(datum)}-${keyGetter(datum)}`), void 0)), 
    ...
    evaluateEncode(elements, encoders, parameters, noGroupEncode) {
        ...
        if (encoders) {
            ...
            elements.forEach((element => {
                ...
                this.isCollectionMark() && (null == groupEncodeAttrs ? void 0 : groupEncodeAttrs[element.groupKey]) && isValid(groupEncodeAttrs[element.groupKey].defined) && (element.items.forEach((item => {
    ...
    prepareRelease() {
        ...
        this.elementMap.forEach((element => element.diffState = DiffState.exit)), this._finalParameters = this.parameters();
    }
    release() {
        ...
        this.graphicItem && removeGraphicItem(this.graphicItem), this.detachAll(), super.release();
    }
}
