...
const vscale_1 = require("@visactor/vscale"), vutils_1 = require("@visactor/vutils"), base_1 = require("../../constant/base"), common_1 = require("../../mark/utils/common"), state_manager_1 = require("../signal/state-manager"), util_1 = require("./util");
...
class MarkStateManager extends state_manager_1.StateManager {
    ...
    checkOneState(renderNode, datum, state, isMultiMark) {
        var _a;
        isMultiMark = (0, vutils_1.isBoolean)(common_1.isMultiDatumMark) ? isMultiMark : !renderNode.mark || (0, 
        common_1.isMultiDatumMark)(renderNode.mark.markType);
        let inState = !1, stateChecked = !1;
        if ((0, vutils_1.isValid)(state.datums) && state.datums.length > 0) inState = this.checkDatumState(state, datum, isMultiMark), 
    ...
    checkState(renderNode, datum) {
        const result = renderNode.getStates().filter((s => (0, util_1.stateInDefaultEnum)(s))).map((s => [ s, 10 ])), isMultiMark = !renderNode.mark || (0, 
        common_1.isMultiDatumMark)(renderNode.mark.markType);
        for (let i = 0; i < this._stateInfoList.length; i++) {
    ...
    checkDatumState(state, datum, isMultiMark) {
        ...
        if ((0, vutils_1.isArray)(state.datums)) {
            const keys = state.datumKeys || Object.keys(state.datums[0]).filter((k => !k.startsWith(base_1.PREFIX)));
            ...
            inState = (state.datumKeys || Object.keys(state.datums).filter((k => !k.startsWith(base_1.PREFIX)))).every((k => {
                var _a, _b;
