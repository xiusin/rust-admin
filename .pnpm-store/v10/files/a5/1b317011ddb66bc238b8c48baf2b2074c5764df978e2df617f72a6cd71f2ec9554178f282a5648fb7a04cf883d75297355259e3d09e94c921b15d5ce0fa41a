...
export class SequenceChartSpecTransformer extends BaseChartSpecTransformer {
    ...
    transformSpec(spec) {
        ...
        let rowNum = 0;
        const elements = [], region = [], axes = [], scrollBar = [], rowHeight = [], viewLayoutRect_height = (this._option.getCompiler().getVGrammarView().width(), 
        this._option.getCompiler().getVGrammarView().height());
